...
QList<QPixmap> *FFmpengWorker::getScreenCaps(QString fileUrl) {
  ...
  // Open video file
  if (avformat_open_input(&pFormatCtx, fileUrl.toStdString().data(), NULL,
                          NULL) != 0)
  ...
  numBytes = avpicture_get_size(AVPixelFormat::AV_PIX_FMT_RGB24,
  ...
  avpicture_fill((AVPicture *)pFrameRGB, buffer,
                 AVPixelFormat::AV_PIX_FMT_RGB24, pCodecCtx->width,
                 pCodecCtx->height);

  // initialize SWS context for software scaling
  sws_ctx = sws_getContext(pCodecCtx->width, pCodecCtx->height,
                           pCodecCtx->pix_fmt, pCodecCtx->width,
                           pCodecCtx->height, AVPixelFormat::AV_PIX_FMT_RGB24,
    ...
    if (packet.stream_index == videoStream) {
      ...
      if (frameFinished) {
        ...
        if (++i <= 5) {
          ...
          QImage img(pCodecCtx->width, pCodecCtx->height,
        ...
        if (frameFinished) {
          ...
          for (int y = 0; y < img.height(); ++y)
            memcpy(img.scanLine(y),
                   ...
                   img.width() * 3);
          // scale it smoothly
          img = img.scaled(QSize(720, 480), Qt::KeepAspectRatio,
                           Qt::SmoothTransformation);
